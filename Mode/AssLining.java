...
public class AssLining extends Mode {
    public AssLining(Manager m) {
        super(m);
        _lining = false;
    }
    public AssLining(Manager m, Coor ep) {
        super(m, ep);
    }
    ...
    public void release(Coor pos) {
        ...
        if(rObj != null) {
            if(rObj instanceof PortObj) {
                ...
                _m.addObj(new AssLine("", depth, pos, sPort, ePort));
            }
        }
        _lining = false;
    }
    ...
    public void rendering(Graphics2D g2) {
        if(_lining){
            AssLine.printLine(_m.getLineStart(), _ep, g2);
        }
    }
}